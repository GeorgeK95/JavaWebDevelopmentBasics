...
import static org.galaxi.handlers.BroccoRequestHandlerCostants.*;

/**
 * Created by George-Lenovo on 6/29/2017.
 */
public class BroccoRequestHandler implements IRequestHandler {
    ...
    public BroccoRequestHandler() {
    }

    public BroccoRequestHandler(String rootPath) {
    ...
    public void handleRequest(InputStream inputStream, DataOutputStream dataOutputStream) {
        try {
            ...
            if (this.request.getUrl().equals(HTML_BROCCO)) {
                this.ok();
    ...
    private byte[] readContent() {
        String path = this.rootPath.concat(STATIC_HTML_INDEX_BROCCO_HTML);
